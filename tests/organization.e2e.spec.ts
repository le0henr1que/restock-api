...
import {
  generateCreateInput,
  generateUpdateInput,
} from './fixtures/organization/organization.fixture.generator';
import { revertOrganizationToInitialState } from './fixtures/organization/organization.test-utils';
import { authenticateUser } from './shared/auth.shared';
import { baseAuthenticationTests } from './shared/base.tests';

describe('Organization module E2E Tests', () => {
  ...
  beforeEach(async () => {
    if (organizationId) {
      await revertOrganizationToInitialState(prisma, organizationId);
    }
  });

  describe('Create Organization', () => {
    const endPointUrl = '/organization';
  ...
  describe('Get filtered Organization', () => {
    const endPointUrl = '/organization';
  ...
  describe('Get Organization by id', () => {
    ...
    it('Should get data', async () => {
      const response = await request(app.getHttpServer())
        .get(`${endPointUrl}/${organizationId}`)
        .set({ Authorization: `Bearer ${bearerToken}` })
        .expect(200);
    ...
    it('Should throw error if Organization id doesnt exist', async () => {
  ...
  describe('Update', () => {
    ...
    it('Should update a Organization', async () => {
      await request(app.getHttpServer())
        .put(`${endPointUrl}/${organizationId}`)
        .set('Authorization', `Bearer ${bearerToken}`)
  ...
  describe('Delete Organization', () => {
    ...
    it('Should inactive a Organization', async () => {
      await request(app.getHttpServer())
        .delete(`${endPointUrl}/${organizationId}`)
        .query({ version: 1 })
        .set('Authorization', `Bearer ${bearerToken}`)
        .expect(200);
    });
